...
export default {
  ...
  data() {
    return {
      ...
      */startedAt: new Date(2021, 0, 23, 15, 0, 0),
      finishedAt: new Date(2021, 0, 23, 17, 0, 0),
      started_at: new Date(2021, 0, 23, 8, 0, 0),
      finished_at: new Date(2021, 0, 23, 10, 0, 0),
      finished_at1: null,
      finished_at2: null,
      step: 0,
      mainStep: 0,
  ...
  methods: {
    ...
    nextStep() {
      ...
      const isTime = today > new Date(this.started_at);
    ...
    createAnswerForm() {
      if (this.stageInformationOfParticipant.document == 1) {
        ...
        this.$store
          ...
          .then((answerForm) => {
            const started_at = new Date(this.started_at);
            const finished_at = new Date(this.finished_at);
            let _answerForm = null;
            if (answerForm != null) {
  ...
  created() {
    ...
    let finished_at2 = new Date(this.finished_at);
    let finished_at1 = new Date(this.finished_at);
