<template>
    ...
    <hr>
    <div class="center">
        <h6>Halo <strong>{{ profilUsernamaTim }}</strong>, kamu terdaftar sebagai peserta MAGE 7 dengan nomor pendaftaran</h6>
        <h4> <strong> {{ profilUsernoPeserta }} </strong></h4>
        ...
        <p>(Penambahan 3 digit terakhir digunakan agar memudahkan dalam konfirmasi)</p>
    </div>
    <hr>
      ...
      <form @submit.prevent="handleSubmit(onSubmit)" enctype="multipart/form-data" > 
        <ValidationProvider name="Nama Pengirim Transfer" rules="required" v-slot="{ errors }">
          <div class="form-group">
            ...
            <input id="namaTim" type="text" class="form-control" v-model="namaBayar">
            <span class="error-msg">{{ errors[0] }}</span>
          </div>
        </ValidationProvider>

        <ValidationProvider name="Bukti Pembayaran" rules="required|ext:jpeg,jpg,png,pdf" v-slot="{ validate, errors }">
          <div class="form-group">
            <label>Bukti Pembayaran</label>
            <input type="file" accept="image/*,application/pdf" class="form-control" @change="{ onUpload($event) || validate($event) }" id="identitasKetua">
            <span class="error-msg">{{ errors[0] }}</span>
          </div>
        </ValidationProvider>
        <p>Catatan : harap mengunggah bukti yang jelas terlihat nama pengirim, nama penerima, nominal, dan tanggal pembayaran.</p>
